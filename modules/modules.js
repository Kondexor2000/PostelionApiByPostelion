...
router.get('',async (req, res) => {
    const credential = await security.asyncCheckCredentials(req.headers['authorization'],db,'modules_read',res);

    if(credential)
    {
        const result = await db('modules').select('id').select('name').select('value').select('icon') .catch(error=>{res.status(500);res.send("Error")});;
